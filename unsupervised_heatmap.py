"""
SKELEX reconstruction and reconstruction-error based hotspot visualization.

Loads a pretrained SKELEX checkpoint, runs inference on every
image in the provided input directories, and saves
  (1) a 4-panel reconstruction figure per image, and
  (2) a multi-pass reconstruction-error heatmap overlay per image.
A per-image mean-error CSV summary is also written under the output root.
"""
...
def show_image(ax: plt.Axes, image_nhwc: torch.Tensor, title: str,
               mean: Sequence[float], std: Sequence[float]) -> None:
    x = image_nhwc
...
def save_reconstruction_panel(out_path: Path,
    ...
    show_image(axs[0], x, "original", mean, std)
    show_image(axs[1], im_masked, "masked", mean, std)
    show_image(axs[2], rec, "reconstruction", mean, std)
    show_image(axs[3], im_paste, "reconstruction + visible", mean, std)
...
def compute_hotspot_map(
    ...
    mean = getattr(processor, "image_mean", [0.485, 0.456, 0.406])
    std = getattr(processor, "image_std", [0.229, 0.224, 0.225])
    x_img = denorm_imagenet(x0, mean, std)
    ...
    for _ in range(int(n_passes)):
        ...
        x_dn = denorm_imagenet(x, mean, std)
        rec_dn = denorm_imagenet(rec, mean, std)
...
def save_hotspot_overlay(out_path: Path,
    ...
    if plot_norm:
        im = ax.imshow(err_map, cmap="hot", alpha=alpha, vmin=0.0, vmax=1.0)
    else:
...
def process_images(
    ...
    for p in tqdm(image_paths, desc="Visualizing"):
        try:
            ...
            save_reconstruction_panel(panel_path, x, rec, mask, processor)
            ...
            rows.append({"path": str(p), "mean_error": mean_err})
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="SKELEX reconstruction & hotspot visualization")
    ap.add_argument("--model-path", type=Path, required=True,
                    help="SKELEX model checkpoint directory (or HuggingFace repo id)")
    ap.add_argument("--in-dirs", type=Path, nargs="+", required=True,
                    help="One or more input directories (searched recursively)")
    ap.add_argument("--out", type=Path, required=True, help="Root output directory")
    ap.add_argument("--format", type=str, default="png", help="Output format: png|jpeg|pdf")
    ap.add_argument("--passes", type=int, default=10,
                    help="Number of forward passes averaged for the hotspot map")
    ap.add_argument("--mask-ratio", type=float, default=0.75,
                    help="Mask ratio kept for record-keeping; the actual ratio is "
                         "controlled by the loaded model config.")
    ap.add_argument("--use-l1", action="store_true",
                    help="Use L1 reconstruction error for the hotspot (default is L2).")
    ap.add_argument("--alpha", type=float, default=0.55, help="Hotspot overlay alpha")
    ap.add_argument("--plot-norm", action="store_true",
                    help="Plot the hotspot on a fixed [0, 1] color range.")
