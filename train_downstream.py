...
python train_downstream.py \
    --data fracatlas \
    --backbones skelex vit-i21k resnet-101 vit-mae biomedclip radio-dino-snarcy \
    --cv 5 \
    --epochs 50 \
    --seed 2025 \
    --test_split 0.1 \
...
def _img_tfm(train: bool, backbone_name: str = "resnet-101"):
    # Default ImageNet values
    mean, std = IMNET_MEAN, IMNET_STD
    size = IMG_SIZE

    if backbone_name == "radio-dino-snarcy":
    ...
    elif "vit-i21k" in backbone_name.lower(): 
        from transformers import AutoImageProcessor
        proc = AutoImageProcessor.from_pretrained("google/vit-large-patch16-224-in21k")
...
class _TimmHFWrapper(nn.Module):
    """Wraps a timm trunk so it returns a HuggingFace-style object exposing `.logits`."""

    _Out = namedtuple("Out", ["logits"])
    ...
    def forward(self, x):
        return self._Out(logits=self.trunk(x))


def build_model(backbone_ckpt: str, num_labels: int, problem: str):
    if "biomedclip" in backbone_ckpt.lower():
        # BiomedCLIP is a ViT-Base/16; load the weights via timm.
        model = timm.create_model(
            "vit_base_patch16_224",
            pretrained=True,
            num_classes=num_labels,
            pretrained_cfg_overlay=dict(
                hf_hub_id="microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224",
                hf_hub_filename="open_clip_pytorch_model.bin",
            ),
        )
        model = _TimmHFWrapper(model).to(DEVICE)

    elif "snarcy" in backbone_ckpt.lower():
        model = timm.create_model(backbone_ckpt, pretrained=True, num_classes=num_labels)
        model = _TimmHFWrapper(model).to(DEVICE)
    
    elif any(k in backbone_ckpt.lower() for k in ("vit_mae", "vit-mae", "skelex")):
...
def build_param_groups_with_llrd(model, base_lr: float, weight_decay: float, layer_decay: float):
    ...
    if hasattr(model, "trunk") and not (hasattr(model, "vit") or hasattr(model, "dinov2")):
        main_model = model.trunk
        is_timm = True
    else:
        main_model = model
        is_timm = False
    is_hf_vit = hasattr(main_model, "vit") and hasattr(main_model.vit, "encoder")
    is_hf_dino = hasattr(main_model, "dinov2") and hasattr(main_model.dinov2, "encoder")

    # If no decay requested or unsupported architecture, return single group
    if layer_decay >= 0.999 or (not is_hf_vit and not is_hf_dino and not is_timm):
        return [{"params": [p for p in model.parameters() if p.requires_grad],
                "lr": base_lr, "weight_decay": weight_decay}]
    
    if is_hf_dino:
        encoder = main_model.dinov2.encoder
        n_layers = len(encoder.layer)
        layer_key = "dinov2.encoder.layer."
    elif is_hf_vit:
        encoder = main_model.vit.encoder
        n_layers = len(encoder.layer)
        layer_key = "vit.encoder.layer."
    else:  # timm / BiomedCLIP
        if hasattr(main_model, "blocks"):
            n_layers = len(main_model.blocks)
            layer_key = "trunk.blocks."
        else:
            n_layers = 0
            layer_key = "__no_layer_key__"
            print(
                f"[warn] LLRD: could not find transformer blocks on "
                f"{type(main_model).__name__}; falling back to single group."
            )

    def depth_from_name(name: str) -> int:
        # Embeddings are always depth 0
        if "embeddings" in name or "patch_embed" in name:
            return 0
        # Check for numbered layers
        if layer_key in name:
            try:
                # Extracts the 'i' from '...layer.i.weight' or '...blocks.i.weight'
                return int(name.split(layer_key)[1].split(".")[0]) + 1
            except (IndexError, ValueError):
                pass
        # Head or Norm layers outside the encoder blocks are 'at the end'
        return n_layers + 1
    ...
    for name, p in model.named_parameters():
        ...
        # Apply the decay formula: lr_i = base_lr * (decay ^ distance_from_head)
        lr = base_lr * (layer_decay ** (n_layers + 1 - d))
        
        # disable weight decay for biases and layer norms
...
def cv_loop(
    ...
    for fold_id, (tr_rel, vl_rel) in enumerate(splitter.split(np.zeros(len(full_ds)), labels), 1):
        ...
        for bk in backbones:
            ...
            tr_ds.tfm = _img_tfm(train=True, backbone_name=bk)
            vl_ds.tfm = _img_tfm(train=False, backbone_name=bk)
            te_ds.tfm = _img_tfm(train=False, backbone_name=bk)
...
def dump_results(outdir: str, tag: str, all_folds: Dict[str, List[dict]], agg: Dict[str, Dict]):
    ...
    print(f"[done] saved results to {Path(outdir).resolve()}")
...
def parse_args():
    ...
    p.add_argument("--data", required=True, choices=SUPPORTED_DATASETS)
    p.add_argument("--backbones", nargs="+", required=True,
                   help="List of backbone aliases to compare (e.g. skelex vit-i21k)")
    ...
    p.add_argument("--label_fraction", type=float, default=1.0, 
                   help="Fraction of the training data to use (0.0 to 1.0) for label efficiency study.")
    return p.parse_args()
...
def main():
    ...
    if args.data == "pesplanus":
        ...
        raise ValueError(f"Unsupported --data value: {args.data!r}")
    ...
    if args.label_fraction < 1.0:
        print(f"--- Label Efficiency: Subsampling {args.label_fraction*100}% of training pool ---")
        
        ss = StratifiedShuffleSplit(n_splits=1, train_size=args.label_fraction, random_state=args.seed)
        
        # Get labels for the training pool only
        tv_labels = y_labels[train_val_idx] if y_labels is not None else None
        
        # Indices relative to train_val_idx
        keep_rel_idx, _ = next(ss.split(np.zeros(len(train_val_idx)), tv_labels))
        train_val_idx = train_val_idx[keep_rel_idx]
    ...
    tag = f"{args.data}_{'-'.join(args.backbones)}_test{int(args.label_fraction * 100)}"
    dump_results(
        args.outdir,
        tag,
        folds,
