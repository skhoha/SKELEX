"""
Multi-headed classifier training script.

This script trains the multi-headed bone tumor classifier fine-tuning the SKELEX model, 
using a combined dataset of SNUH-bonetu, BTXRD-center-1, andFracAtlas.
...
def _group_confusion_mtx_mc(y_true, y_prob, y_mask, idxs0):
    ...
    """
    idxs0 = np.asarray(idxs0, dtype=int)  
...
def main():
    ...
    for fold_id, (tr_rel, va_rel) in enumerate(kfold.split(remain_idx_np), start=1):
        ...
        model_name = f"multiheadclassifier_fold{fold_id}"
